...
def main():
    ...
    if options.regex is not None:
        ...
        try:
            ...
            req = urllib.request.Request(options.url)
        ...
        try:
            ...
            with zipfile.ZipFile(installer_path, 'r') as zip_ref:
                for zip_info in zip_ref.infolist():
                    extracted_path = Path(unpack_path, zip_info.filename)

                    # Detect if this should be a symlink
                    if zip_info.external_attr >> 28 == 0xA:  # Symlink in POSIX (0xA)
                        symlink_target = zip_ref.read(zip_info.filename).decode()
...
def install_pkg(pkg_path: Path, unpack_path: Path):
    ...
    if distribution_file.exists():
        ...
        try:
            ...
            package_id = root.attrib.get('identifier')
...
if __name__ == '__main__':
    ...
    # Use a regex
    basics_group.add_argument('-r', '--regex', default=None,
