...
#
# Description
#   Download a package and install for website
#
# Versions
# 1.	Find the download and install the file
...
for arg in "$@"; do
    if [[ "$arg" == "-h" || "$arg" == "--help" ]]; then
        echo "$BASENAME <not used> <not used> <not used> <url to download> <regex> <optional: install type dmg,pkg,zip,tar>"
        echo "Example:"
        echo "$BASENAME _ _ _ https://example.com/downloads" 'https:[\/A-z0-9\.]+v(\d+\.)+\d+\/myapp-(\d\.)+pkg'
        exit 0
    fi
done
...
PATTERN=${5}
[ "$PATTERN" == "" ] && logging.critical "No regex pattern"

#  Curl the page containing the URI
HTML="$(curl -sL $URL)"
# Find the url
URL="$(echo "$HTML" | /usr/bin/egrep -o "$PATTERN" | head -n 1)"
